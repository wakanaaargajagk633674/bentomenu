...
生成は2段階です。最初は4候補について料理名、構成、味、原価、特徴だけを返します。利用者が1件を選ぶと、その1件だけ詳細レシピ、手順、品質審査、盛り付け仕様を生成し、検証済みレシピから完成写真を1枚生成します。完成写真は費用を抑えるためGPT Image 2の1024×1024・low品質（画像出力の公式見積 $0.006、入力費用は別途）を使用します。写真生成に失敗した場合は、その写真だけ再試行できます。画像はAIによる盛り付け参考であり、調理時はレシピの加熱・冷却・保冷指示を優先してください。
