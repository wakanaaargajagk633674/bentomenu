...
export default function IzakayaPage() {
  ...
  return <main className="planner-page izakaya-planner">
    ...
    <section className="planner-form" aria-label="日替わり逸品の条件">
      ...
      {isGenerating && <div className="generation-progress" role="status"><div className="progress-top"><span className="progress-spinner" /><div><b>料理人チームが考えています</b><p>{stage}</p></div><time>{elapsed}秒</time></div><div className="progress-track"><i /></div><div className="progress-foot"><span>通常1〜3分ほどかかります。逸品案を先に表示し、写真はその後に生成します。</span><button type="button" onClick={() => abortRef.current?.abort()}>中止する</button></div></div>}
