...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({ suggestions: response.output_parsed.suggestions.map((suggestion) => {
      ...
      const normalized = { ...suggestion, basePrice: input.data.price, menuType: "daily-special" as const, profitPlan: { ...suggestion.profitPlan, totalVariableCostYen, estimatedGrossProfitYen, variableCostRatePercent } };
      return { ...normalized, imageToken: signIzakayaSuggestion(normalized) };
    }) });
  } catch (error) {
