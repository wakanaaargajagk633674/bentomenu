...
export default function HomeBentoPage() {
  ...
  const summary = useMemo(() => `${homeBentoAgeLabels[ageGroup]}・${homeBentoGenderLabels[gender]}・${largePortion ? "量多め" : "標準量"}・予算${budgetYen.toLocaleString()}円`, [ageGroup, gender, largePortion, budgetYen]);
  ...
  const generateCandidates = async () => {
    ...
    const submitted: HomeBentoRequest = { ageGroup, gender, largePortion, budgetYen };
    setElapsedSeconds(0);
  ...
  return <main className="planner-page home-bento-planner">
    ...
    <section className="planner-hero home-bento-hero">
      ...
      <p>年齢・性別・食べる量・1食予算から、家庭の台所で無理なく作れる4案を提案。冷めてもおいしく、安全で、家庭用弁当箱にきれいに収まるレシピと写真を作ります。</p>
    ...
    <section className="planner-form" aria-label="家庭用弁当の条件">
      <div className="form-heading"><div><p className="eyebrow">FAMILY CONDITIONS</p><h2>4つの条件を選択</h2></div><p>販売価格や利益ではなく、1食分の食材予算で考えます</p></div>
