...
const HOME_SEASON_RULES = `季節は旬の食材名だけで表現せず、気候、香り、色、切り方、冷めた食感、安全へ必然的に反映する。春は若い苦味・淡い色・香り、夏は酸味・香味・水分制御と保冷、秋は実り・きのこ・根菜・香ばしさ、冬は根菜・発酵・煮含めた味を軸にする。ただし弁当は常温で脂が固まる重い料理や汁気を避ける。旬食材が予算や地域で入手しにくい場合は、通年品の技法・香り・色で季節を表現し、無理な高級食材を使わない。`;
...
各候補のbudgetYenとtargetAgeGroupは入力と一致させ、seasonは「採用する季節」と一致させる。seasonalDesignには旬、気候、香り、色、冷めた食感、安全への反映を短く具体的に書く。budgetPlanは主材料、野菜・副菜、主食・調味料を円単位で見積もり、totalEstimatedYenを3項目の合計、remainingYenを予算上限との差にし、必ず予算内にする。朝の再現性、冷めたおいしさ、年齢に適した安全な一口サイズを候補段階から守る。`;
}
